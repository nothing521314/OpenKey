...
bool isSpotlightActive() {
    ...
    if (app && [app.bundleIdentifier isEqualToString:@"com.apple.Spotlight"]) {
        return true;
    }
    return false;
}

bool ReplaceFocusedTextViaAX(NSString *newString, int backspaceCount) {
    AXUIElementRef systemWide = AXUIElementCreateSystemWide();
    AXUIElementRef focusedElement = NULL;
    AXError err = AXUIElementCopyAttributeValue(systemWide, kAXFocusedUIElementAttribute, (CFTypeRef *)&focusedElement);
    CFRelease(systemWide);

    if (err != kAXErrorSuccess || !focusedElement) return false;
    ...
    NSString *currentValue = (__bridge NSString *)valueRef;

    CFTypeRef rangeRef = NULL;
    err = AXUIElementCopyAttributeValue(focusedElement, kAXSelectedTextRangeAttribute, &rangeRef);
    CFRange range = {0, 0};
    if (err == kAXErrorSuccess && rangeRef && CFGetTypeID(rangeRef) == AXValueGetTypeID()) {
        AXValueGetValue((AXValueRef)rangeRef, kAXValueTypeCFRange, &range);
    }
    if (rangeRef) CFRelease(rangeRef);

    NSUInteger location = range.location;
    if (location == kCFNotFound) location = currentValue.length;

    NSUInteger deleteLength = backspaceCount;
    if (deleteLength > location) deleteLength = location;

    NSRange deleteRange = NSMakeRange(location - deleteLength, deleteLength);
    
    NSMutableString *mutableValue = [currentValue mutableCopy];
    if (deleteRange.location != NSNotFound && deleteRange.location + deleteRange.length <= mutableValue.length) {
         [mutableValue replaceCharactersInRange:deleteRange withString:newString];
    } else {
        CFRelease(valueRef);
        CFRelease(focusedElement);
        return false;
    ...
    err = AXUIElementSetAttributeValue(focusedElement, kAXValueAttribute, (__bridge CFTypeRef)mutableValue);
    
    if (err == kAXErrorSuccess) {
        CFRange newRange = {(CFIndex)(deleteRange.location + newString.length), 0};
...
extern "C" {
    ...
    void SendNewCharString(const bool& dataFromMacro=false, const Uint16& offset=0) {
        if (isSpotlightActive()) {
            ...
            int count = dataFromMacro ? (int)pData->macroData.size() : pData->newCharCount;
            
            if (dataFromMacro) {
                for (int i = 0; i < count; i++) {
                    Uint32 data = pData->macroData[i];
                    if (data & PURE_CHARACTER_MASK) {
                        [s appendFormat:@"%C", (UniChar)(data & 0xFFFF)];
                    } else if (!(data & CHAR_CODE_MASK)) {
                        UniChar c = keyCodeToCharacter(data);
                        if (c) [s appendFormat:@"%C", c];
                    } else {
                        [s appendFormat:@"%C", (UniChar)(data & 0xFFFF)];
                    }
                }
            ...
            if ((pData->code == vRestore || pData->code == vRestoreAndStartNewSession) && keyCodeToCharacter(_keycode) != 0) {
                 UniChar c = keyCodeToCharacter(_keycode | ((_flag & kCGEventFlagMaskAlphaShift) || (_flag & kCGEventFlagMaskShift) ? CAPS_MASK : 0));
                 if (c) [s appendFormat:@"%C", c];
            }

            NSString *finalString = [s precomposedStringWithCanonicalMapping];
            
            if (ReplaceFocusedTextViaAX(finalString, pData->backspaceCount)) {
            ...
            for (int i = 0; i < pData->backspaceCount; i++) {
                ...
                usleep(8000);
                CFRelease(bsDown); CFRelease(bsUp);
            }
            
            NSUInteger len = [finalString length];
            unichar buffer[len+1];
            [finalString getCharacters:buffer range:NSMakeRange(0, len)];
            
            for (int i = 0; i < len; i++) {
                UniChar ch = buffer[i];
                CGEventRef eDown = CGEventCreateKeyboardEvent(myEventSource, 0, true);
                CGEventRef eUp = CGEventCreateKeyboardEvent(myEventSource, 0, false);
                CGEventKeyboardSetUnicodeString(eDown, 1, &ch);
                CGEventKeyboardSetUnicodeString(eUp, 1, &ch);
                CGEventPost(kCGHIDEventTap, eDown);
                usleep(8000);
                CGEventPost(kCGHIDEventTap, eUp);
                usleep(8000);
                CFRelease(eDown); CFRelease(eUp);
            }
